...
% Auto-detect OS and use appropriate CJK font
\usepackage{ifplatform}
\ifwindows
  \setCJKmainfont{SimSun}           % Windows: SimSun (built-in)
\else
  \ifmacosx
    \setCJKmainfont{Songti SC}      % macOS: Songti SC (built-in)
  \else
    \setCJKmainfont{Noto Serif CJK SC}  % Linux: Noto Serif CJK SC
  \fi
\fi
...
\begin{frame}{研究概览：问题、方法与核心发现}
  ...
  \medskip
  \centering
  \includegraphics[height=0.32\textheight,keepaspectratio]{images/membrane-pore-jc/mermaid.png}
\end{frame}
...
\begin{frame}{Full-Path结果：正反向拉伸完全重合，CV设计可逆无滞后}
  \iqblayouttwothirds{
    ...
    \centering
    \includegraphics[height=0.75\textheight,keepaspectratio]{images/membrane-pore-jc/fig4.png}
...
\begin{frame}{Rapid方法：脂质条带模拟"无限孔"快速提取线张力}
  \iqblayouttwothirds{
    ...
    \centering
    \includegraphics[height=0.8\textheight,keepaspectratio]{images/membrane-pore-jc/fig5ab.png}
...
\begin{frame}{离子效应：$\ce{Ca^{2+}}$恢复被$\ce{Na+}$降低的膜线张力}
  \iqblayouttwo{
    ...
    \centering
    \includegraphics[height=0.7\textheight,keepaspectratio]{images/membrane-pore-jc/fig7.png}
  }
\end{frame}
