...
\iqbformulaframe{Full-Path：切换函数巧妙结合成核与扩展}{
  ...
  \tinysep
  \iqbcaptiontext{\textbf{图1}：两种创新的集体变量设计。\textbf{上半部}：Full-Path CV同时追踪成核（圆柱内尾部密度CV$_{\text{cyl}}$）和扩展（孔半径CV$_{\text{r}}$）；通过sigmoid切换函数平滑过渡。\textbf{下半部}：Rapid CV使用脂质条带模拟无限孔，通过调节盒子尺寸快速提取线张力（因子2考虑两孔边缘）。}
}
