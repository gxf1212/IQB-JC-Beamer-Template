...
\begin{frame}{超半数酶为同源二聚体：一个不容忽视的生物学现象}
  \iqblayoutcustom[0.30]{
    ...
    \begin{iqbitemize}
      \item UniProt数据库：同源二聚体占所有蛋白质结构的51\%以上，远超单体和其他寡聚形式
      \item Brenda数据库：已定义寡聚形式的酶中，二聚体数量超过单体
      \item 偶数亚基($2n$)酶多于奇数亚基($2n-1$)酶
      \item 大多数辅因子依赖酶(NAD, ATP, 金属离子等)以同源二聚体形式存在
    \end{iqbitemize}

  }{
    \iqbfig[height=0.50\textheight]{Figures/Figure01.png}{从19世纪统计力学基础到现代生物分子模拟：同源二聚体酶的演进与普遍性}
    \iqbtinysep

    \iqborangebox{关键发现}{各种催化功能(转移酶、裂合酶、水解酶、连接酶、氧化还原酶、异构酶)中，同源二聚体形式均占50\%以上}
  }
\end{frame}
...
\begin{frame}{经典理论奠定基础，现代技术揭示机制}
  \iqblayouttwo{
    ...
    \begin{iqbitemize}
      \item 1965年：Monod-Wyman-Changeux (MWC)模型为血红蛋白建立变构与协同性理论
      \item Perutz的双结构立体化学模型为理解二聚体酶亚基间协同性提供基础
      \item 亚基间变构调节导致底物/产物亲和力或催化效率差异，通过构象变化实现
    \end{iqbitemize}
  }{
    ...
    \iqbgreenbox{应用前景}{为设计高效催化剂(如二聚体有机催化剂)和药物(如Paxlovid靶向SARS-CoV-2同源二聚体主蛋白酶)提供理论支撑}
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}{二聚化的生物学优势Ia：促进蛋白质折叠}
  ...
  \iqblayouttwo{
    ...
    \begin{iqbitemize}
      \item 亚基界面提供额外的疏水核心
      \item 减少溶剂暴露表面积(BSA降低)
      \item 降低折叠能量势垒$\Delta G_{fold}$
      \item 提高折叠效率和成功率
      \item 减少聚集形成包涵体的风险
    \end{iqbitemize}
    ...
    \begin{iqbitemize}
      \item 疏水效应：疏水残基埋藏释放水分子
      \item 焓驱动：界面氢键、盐桥形成
      \item 熵驱动：溶剂化水分子释放增加熵
      \item 总自由能：$\Delta G_{bind} = \Delta H - T\Delta S$
    \end{iqbitemize}
  }{
    \iqbsectiontitle{定量数据}
    \begin{iqbitemize}
      \item 典型界面埋藏面积：1000-3000 Å²
      \item 界面自由能：-5至-15 kcal/mol
      \item 解离常数：$K_d$ = nM至$\mu$M级别
      \item BSA减少：15-25\%
    \end{iqbitemize}
...
% ------------------------------------------------------------
\begin{frame}{二聚化的生物学优势Ib2：界面相互作用网络}
  \iqbfontsizemode{small}
  ...
  \iqblayouttwo{
    ...
    \begin{iqbitemize}
      \item 氢键：每个2-5 kcal/mol
      \item 盐桥：每对3-7 kcal/mol
      \item 疏水作用：总10-20 kcal/mol
      \item 芳香堆积：每对2-4 kcal/mol
    \end{iqbitemize}
  }{
    ...
    \begin{iqbitemize}
      ...
      \item 优化疏水核心降低解折叠速率
      \item 添加二硫键锁定二聚体构象
      \item 成功案例：嗜热菌酶$T_m$提升至95°C
    \end{iqbitemize}
  }

  \iqbtinysep

  {\tiny\iqborangebox{工程策略}{多重界面相互作用协同提升热稳定性}}
\end{frame}

% ------------------------------------------------------------
% 4b. Biological Advantages: Catalysis & Regulation
% ------------------------------------------------------------
\begin{frame}{二聚化的生物学优势II：催化与调控}
  \iqbfontsizemode{small}
  ...
  \iqblayouttwo{
    ...
    \begin{iqbitemize}
      ...
      \item Caspase-9等凋亡酶通过二聚化激活
      \item 实现开关式调控：ON/OFF状态
      \item 避免单体在细胞内误触发反应
      \item 提供额外调控层次
    \end{iqbitemize}
  }{
    ...
    \begin{iqbitemize}
      \item 催化效率提升：2-1000倍
      \item 底物亲和力提高：$K_M$降低10-100倍
      \item 产物释放加速：$k_{off}$提高
      \item 协同系数：Hill系数$n_H > 1$
    \end{iqbitemize}
    ...
    \iqborangebox{核心优势}{二聚化整合结构稳定、催化增强、调控精准}
  }
\end{frame}
...
\begin{frame}{MWC模型的数学框架：状态、权重与反应速率}
  ...
  \iqblayoutcustom[0.45]{
    ...
    \begin{iqbitemize}
      \item 酶在两种构象状态间平衡：活性态($E_A$)和非活性态($E_I$)
      \item 对称性：二聚体的两个亚基同时处于相同状态
      \item 预存平衡：构象转换发生在底物结合之前
      \item 活性态对底物的亲和力远高于非活性态
    \end{iqbitemize}
...
\begin{frame}{蛋白质运动的时间尺度：从飞秒到秒}
  ...
  \iqblayoutcustom[0.35]{
    ...
    \iqbbluebox{MD挑战}{催化运动跨越6-9个数量级，需增强采样}
  }
\end{frame}

% 4a2-1a. Thermostability: Quantitative Data
% ------------------------------------------------------------
\begin{frame}{二聚化的生物学优势Ib1-1：定量热稳定性数据}
  \iqbfontsizemode{small}

  \iqblayoutcustom[0.35]{
    \iqbsectiontitle{定量数据}
    \begin{iqbitemize}
      \item 二聚体$T_m$比单体高10-30°C
      \item 典型界面埋藏面积：1000-3000 Å²
      \item 每100 Å²界面约提升$T_m$ 1-2°C
      \item 解折叠自由能提升5-15 kcal/mol
      \item 蛋白酶解抗性提高2-10倍
    \end{iqbitemize}
  }{
...
% ------------------------------------------------------------
\begin{frame}{二聚化的生物学优势Ib1-2：稳定机制}
  \iqbfontsizemode{small}
  ...
  \iqblayouttwo{
    ...
    \begin{iqbitemize}
      \item 氢键：2-5 kcal/mol，提供特异性
      \item 盐桥：3-7 kcal/mol，稳定界面
      \item 疏水作用：主要驱动力，占60-80\%
      \item 范德华力：优化界面互补性
    \end{iqbitemize}
  }{
...
\begin{frame}{生物物理实验方法：结构解析与动力学检测}
  \iqblayouttwo{
    ...
    \iqborangebox{多技术结合}{结构+动力学+热力学的组合实验揭示完整的催化机制}
  }
\end{frame}
...
\begin{frame}{变构理论：MWC构象选择 vs. KNF诱导契合}
  \iqblayouttwo{
    \iqbsectiontitle{MWC模型(Conformational Selection)}
    \begin{iqbitemize}
      \item 配体选择性结合与底物结合状态相似的主导构象
      \item 强调亚基间对称性维持
      \item 预先存在多种离散构象的平衡
    ...
    \iqbsectiontitle{KNF模型(Induced Fit)}
    \small
    \begin{iqbitemize}
      \item 配体随机结合后诱导构象转变至最优态
      \item 允许亚基间非对称性
    ...
    \iqbbluebox{现代观点}{动力学研究表明，构象选择(MWC)可能是配体结合目标蛋白的更普遍机制}
  }
\end{frame}
...
\begin{frame}{变构调节的热力学本质：熵与焓的平衡}
  ...
  \iqblayoutcustom[0.38]{
    ...
    \begin{iqbitemize}
      \item 变构不仅是构象改变，更依赖于熵和焓的综合变化
      \item Cooper \& Dryden开创性发现：配体结合改变蛋白质热涨落幅度，而非构象本身
      \item "熵驱动"系统：刚性和柔性区域的再平衡
      \item PKA-C等酶的构象熵变化全局分布于整个酶
    \end{iqbitemize}
    ...
    \begin{iqbitemize}
      \item $\Delta G = \Delta H - T\Delta S$：自由能驱动变构
      \item 焓变：氢键、盐桥、范德华力的重排
      \item 熵变：构象自由度、溶剂化熵的变化
      \item 温度依赖性：$T\Delta S$项随温度显著变化
    \end{iqbitemize}
  }{
    \iqbfig[height=0.68\textheight]{Figures/Figure07.png}{变构调节：熵-焓补偿原理驱动构象选择与自由能平衡}
  }
\end{frame}
...
\begin{frame}{半位点反应性：为何只有一个亚基催化？}
  ...
  \iqblayoutcustom[0.37]{
    ...
    \begin{iqbitemize}
      \item 二聚体中只有一个位点结合并催化底物
      \item 另一亚基无法结合或结合但不催化
    ...
    \iqbgreenbox{关键概念}{底物结合亲和力变化不等同于催化效率提升，需区分结合协同性与催化协同性}
  }{
...
\begin{frame}{负协同性的四种分子机制}
  \iqblayouttwo{
    ...
    \iqborangebox{生物学意义}{负协同性确保底物识别的特异性，避免非特异性催化}
  }
\end{frame}
...
\begin{frame}{药物设计实例：靶向SARS-CoV-2主蛋白酶的Paxlovid}
  \iqblayoutcustom[0.4]{
    ...
    \begin{iqbitemize}
      ...
      \item 催化三联体：His41-Cys145-Glu166
      \item 二聚化对催化活性至关重要
      \item 单体几乎无活性
    \end{iqbitemize}
    ...
    \small
    \begin{iqbitemize}
      \item 共价抑制剂，与Cys145形成共价键
      \item 利用二聚体活性位点的结构特征
      \item 理解二聚体界面和协同机制指导优化
...
\begin{frame}{正协同性的生物物理特征：实验表征与理论预测}
  ...
  \iqblayoutcustom[0.35]{
    ...
    \begin{iqbitemize}
      \item Hill系数$n_H$：衡量协同性的定量指标
      \item $n_H > 1$：正协同性；$n_H < 1$：负协同性；$n_H = 1$：无协同性
      \item 理论最大值：$n_H = n$(亚基数)，二聚体最大为2
    ...
    \begin{iqbitemize}
      \item S型(sigmoid)底物浓度-反应速率曲线
      \item 陡峭的响应曲线：生理调控优势
      \item $K_{0.5}$：达到半最大速率的底物浓度
  ...
  \iqbmicrosep
    \iqborangebox{生物学意义}{正协同性Hill系数大于1使酶快速响应浓度变化实现代谢开关调控}
  }
\end{frame}
...
\begin{frame}{催化协同性实例：SaMTAN的双重调控机制}
  ...
  \iqblayoutcustom[0.4]{
    ...
    \begin{iqbitemize}
      \item S-腺苷甲硫氨酸(SAM)依赖的甲基转移酶
      \item 催化亚硝基合成，参与细菌致病性
      \item 同源二聚体结构，每个亚基含独立催化位点
      \item 底物结合协同性：$n_H \approx 1.8$
    \end{iqbitemize}
    ...
    \begin{iqbitemize}
      \item 一个亚基结合底物后，通过界面氢键网络
      \item 传递构象变化至相邻亚基
      \item 促进第二个底物结合和催化活性提升
      \item 关键残基突变消除协同性($n_H \rightarrow 1$)
    \end{iqbitemize}

    \iqbtinysep

%%    {\tiny\iqborangebox{结构-功能}{氢键网络介导亚基间协同催化}}
  }{
    \iqbfig[height=0.50\textheight]{Figures/Figure14.png}{SaMTAN氢键网络介导正协同性：界面残基传递构象变化提升催化}
  }
\end{frame}
...
\begin{frame}{亚基功能不对称：ADAR2的结构-功能分工}
  ...
  \iqblayoutcustom[0.4]{
    ...
    \begin{iqbitemize}
      \item 腺苷脱氨酶(ADAR2)：将RNA中A转换为I(肌苷)
      \item 同源二聚体，但两个亚基功能不对称
      \item 一个亚基负责催化，另一个负责RNA识别和定位
      \item 协同作用：提高编辑特异性和效率
    \end{iqbitemize}
    ...
    \begin{iqbitemize}
      \item 晶体结构揭示两个亚基构象差异
      \item RNA结合导致一个亚基活性位点优化
      \item 另一个亚基通过变构效应稳定复合物
      \item 动态不对称：亚基间催化与辅助功能可交替
    \end{iqbitemize}
  }{
    \iqbfig[height=0.38\textheight]{Figures/Figure15.png}{ADAR2二聚体的功能不对称：催化与识别动态交替}
...
\begin{frame}{底物通道化实例：组氨酸生物合成酶HisFH}
  ...
  \iqblayoutcustom[0.4]{
    ...
    \begin{iqbitemize}
      \item 催化组氨酸生物合成中的两步连续反应
      \item 第一步：环化反应生成中间产物PRFAR
      \item 第二步：异构化生成ProFAR
      \item 二聚体结构：两个亚基协同工作
    \end{iqbitemize}
    ...
    \begin{iqbitemize}
      \item 亚基间形成约20Å的封闭通道
      \item PRFAR从一个亚基的催化位点直接传递至另一个亚基
      \item 通道内疏水性环境稳定极性中间产物
      \item 避免中间产物扩散至溶液中降低反应效率
      \item 通道出口处氨基酸残基精确定位底物
    \end{iqbitemize}
  }{
...
\begin{frame}{去甲基化酶JMJD1A：底物通道机制}
  ...
  \iqblayoutcustom[0.40]{
    ...
    \begin{iqbitemize}
      \item Jumonji C (JmjC)家族去甲基化酶
      \item 催化组蛋白H3K9me3 → me2 → me1 → me0
      \item 同源二聚体，Fe(II)和$\alpha$-酮戊二酸依赖性
      \item 每步去甲基化生成甲醛和琥珀酸
    \end{iqbitemize}
    ...
    \begin{iqbitemize}
      \item 中间产物me1通过亚基间通道传递
      \item 防止部分去甲基化的组蛋白提前释放
      \item 确保完全去甲基化(me3 → me0)一步到位
      \item 避免染色质结构异常和表观遗传信息错误
    \end{iqbitemize}
  }
\end{frame}
...
\begin{frame}{质子线通讯：亚基间质子传递网络}
  ...
  \iqblayoutcustom[0.35]{
    ...
    \iqbfig[height=0.6\textheight]{Figures/Figure19.png}{质子线网络：氢键链介导质子传递与协同}
...
\begin{frame}{电子传递协同：细胞色素bc$_1$复合物}
  ...
  \iqblayoutcustom[0.42]{
    ...
    \iqbfig[height=0.64\textheight]{Figures/Figure20.png}{Cytbc$_1$二聚体的电子传递协同：ISP摆动实现跨亚基电子传递}
  }
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}{负热容机制IIa-1a：底物结合的热容变化}
  \iqbfontsizemode{small}
...
\begin{frame}{负热容机制：分子机制与熵-焓补偿}
  ...
  \iqblayouttwo{
    ...
    \begin{iqbitemize}
      ...
      \item 每100 Å²界面约-0.2 kcal/(mol·K)
...
\begin{frame}{底物结合-产物释放耦合：交替位点催化模型}
  ...
  \iqblayoutcustom[0.30]{
    ...
    \iqbfig[height=0.50\textheight]{Figures/Figure23.png}{底物-产物耦合机制：一个亚基底物结合促进另一亚基产物释放}

    \iqbtinysep

    \iqbgreenbox{催化效率倍增}{耦合超过单体加和}
  }
\end{frame}
...
\begin{frame}{亚基界面Ib2：质子传递机制}
  ...
  {\tiny\iqborangebox{协同效应}{质子传递速率保证催化协同性}}
\end{frame}

% ------------------------------------------------------------
% 14b. Interface Importance: Communication Mechanisms Part 2
% ------------------------------------------------------------
\begin{frame}{亚基界面Ib2：电子与静电场传递机制}
  \iqbfontsizemode{small}
...
% ------------------------------------------------------------
\begin{frame}{亚基界面Ib2：水分子网络的动态作用}
  \iqbfontsizemode{small}
...
\begin{frame}{致谢与参考文献}
  ...
  \iqbgreenbox{综述贡献}{系统总结了二聚体酶协同性和动力学的最新研究进展，为酶催化机制研究和催化剂设计提供了全面的理论框架}
